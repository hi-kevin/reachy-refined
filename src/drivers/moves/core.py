...
class MovementManager:
    ...
    def working_loop(self) -> None:
        ...
        logger.debug("Move worker thread started")
        ...
        while not self._stop_event.is_set():
            ...
            # 1. Update frequency stats
            self._freq_stats = self._update_frequency_stats(loop_start, prev_loop_start, self._freq_stats)
            prev_loop_start = loop_start
            
            # 2. Process external signals (offsets, commands)
            self._poll_signals(loop_start)
            self._update_face_tracking(loop_start)
            
            # 3. Update internal movement state (queue, breathing)
            self._update_primary_motion(loop_start)
            
            # 4. Compose final pose
            head_pose, antennas, body_yaw = self._compose_full_body_pose(loop_start)
            
            # 5. Blend antennas (listening freeze logic)
            antennas = self._calculate_blended_antennas(antennas)
            
            # 6. Issue command to robot
            self._issue_control_command(head_pose, antennas, body_yaw)
            
            # 7. Publish state for other threads
            self._publish_shared_state()
            self._record_frequency_snapshot(self._freq_stats)
            self._maybe_log_frequency(loop_count, 500, self._freq_stats)
            
            # 8. Sleep to maintain frequency
            sleep_time, self._freq_stats = self._schedule_next_tick(loop_start, self._freq_stats)
            time.sleep(sleep_time)
            loop_count += 1
            
        logger.debug("Move worker thread exited")
