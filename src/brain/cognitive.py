...
class CognitiveBrain:
    ...
    def __init__(self, robotics_brain: Optional[RoboticsBrain] = None, memory_server: Optional[MemoryServer] = None, api_key: Optional[str] = None):
        ...
        self._initial_turn_done = asyncio.Event()
    ...
    async def start_up(self):
        """Initialize connection to Gemini Live."""
        self.running = True
        logger.info(f"Cognitive Brain starting up (Model: {MODEL_ID})...")
        ...
        # Configuration using explicit types for v1beta compatibility
        config = types.LiveConnectConfig(
            ...
            # Reverted VAD config to fix "Extra inputs" error
            # This restores the state where at least the first turn worked
        )

        try:
             # Create the session
            # Pass explicit config object
            ...
            self.running = False
            logger.info("Cognitive Brain shutdown.")

    async def _send_loop(self):
        """Consume audio from local stream and send to Gemini."""
        logger.info("[SEND] Starting audio streaming to Gemini immediately (no greeting).")
        ...
        while self.running and self._session:
            try:
                    ...
                    if model_turn:
                        for part in model_turn.parts:
                            if part.inline_data:
                                ...
                                logger.info(f"[RECV] Text: {part.text[:100]}")
                            else:
    ...
    async def receive(self, frame: Tuple[int, np.ndarray]):
        ...
        if self.running:
            # logger.debug(f"Cognitive received frame: {len(frame[1])} samples")
